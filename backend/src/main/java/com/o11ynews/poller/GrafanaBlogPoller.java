...
/**
 * Polls the Grafana Labs Blog RSS feed.
 * Feed URL: https://grafana.com/blog/index.xml
 * Grafana blog entries embed images in HTML-encoded description content.
 */
@Component
@ConditionalOnProperty(name = "poller.grafanablog.enabled", havingValue = "true", matchIfMissing = true)
public class GrafanaBlogPoller extends BasePoller {

    private static final String FEED_URL = "https://grafana.com/blog/index.xml";

    public GrafanaBlogPoller(ArticleRepository articleRepository,
    ...
    @Override
    @WithSpan("poller.grafanablog.fetch")
    ...
    List<RawArticle> fetchArticles() {
        try {
            ...
            log.error("Failed to fetch Grafana blog feed: {}", e.getMessage(), e);
            return List.of();
        }
    }
    ...
    private List<RawArticle> parseFeed(String feedUrl) throws Exception {
        ...
        for (SyndEntry entry : feed.getEntries()) {
            ...
            // Grafana feed uses relative links — make absolute
            if (url.startsWith("/")) url = "https://grafana.com" + url;
            ...
            if (entry.getDescription() != null && entry.getDescription().getValue() != null) {
                ...
                contentSnippet = html.replaceAll("<[^>]+>", "")
                                     .replaceAll("&[a-zA-Z]+;", " ")
                                     .strip();
            ...
            metadata.put("feed", "grafana.com/blog/index.xml");
    ...
    @Scheduled(fixedDelayString = "${poller.grafanablog.interval:3600}000")
