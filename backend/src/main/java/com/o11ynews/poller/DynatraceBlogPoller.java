...
/**
 * Polls the Dynatrace Blog RSS feed.
 * Feed URL: https://www.dynatrace.com/news/blog/feed/
 */
@Component
@ConditionalOnProperty(name = "poller.dynatraceblog.enabled", havingValue = "true", matchIfMissing = true)
public class DynatraceBlogPoller extends BasePoller {

    private static final String FEED_URL = "https://www.dynatrace.com/news/blog/feed/";

    public DynatraceBlogPoller(ArticleRepository articleRepository,
    ...
    @Override
    @WithSpan("poller.dynatraceblog.fetch")
    ...
    List<RawArticle> fetchArticles() {
        try {
            ...
            log.error("Failed to fetch Dynatrace blog feed: {}", e.getMessage(), e);
            return List.of();
        }
    }
    ...
    private List<RawArticle> parseFeed(String feedUrl) throws Exception {
        ...
        for (SyndEntry entry : feed.getEntries()) {
            ...
            metadata.put("feed", "dynatrace.com/news/blog/feed/");
    ...
    @Scheduled(fixedDelayString = "${poller.dynatraceblog.interval:3600}000")
