...
/**
 * Polls the Datadog Engineering Blog RSS feed.
 * Feed URL: https://www.datadoghq.com/feed.xml
 * No keyword filtering — the entire feed is observability/DevOps content.
 */
@Component
@ConditionalOnProperty(name = "poller.datadogblog.enabled", havingValue = "true", matchIfMissing = true)
public class DatadogBlogPoller extends BasePoller {

    private static final String FEED_URL = "https://www.datadoghq.com/feed.xml";

    public DatadogBlogPoller(ArticleRepository articleRepository,
    ...
    @Override
    @WithSpan("poller.datadogblog.fetch")
    ...
    List<RawArticle> fetchArticles() {
        try {
            ...
            log.error("Failed to fetch Datadog blog feed: {}", e.getMessage(), e);
            return List.of();
        }
    }
    ...
    private List<RawArticle> parseFeed(String feedUrl) throws Exception {
        ...
        for (SyndEntry entry : feed.getEntries()) {
            ...
            metadata.put("feed", "datadoghq.com/feed.xml");
    ...
    @Scheduled(fixedDelayString = "${poller.datadogblog.interval:3600}000")
